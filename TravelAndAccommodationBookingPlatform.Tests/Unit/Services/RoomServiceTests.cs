...
public class RoomServiceTests
{
    ...
    public async Task GetAll_ShouldReturnMappedRoomsAndPagination()
    {
        ...
        _roomRepositoryMock.Setup(r => r.GetAll(queryParams))
        ...
        // Act
        var (result, meta) = await _roomService.GetAll(queryParams);
    ...
    public async Task Create_ShouldReturnRoomDto_WhenSuccessful()
    {
        ...
        _hotelRepositoryMock.Setup(r => r.GetById(1)).ReturnsAsync(hotel);
        _mapperMock.Setup(m => m.Map<Room>(creationDto)).Returns(room);
        _roomRepositoryMock.Setup(r => r.Create(It.IsAny<Room>())).ReturnsAsync(room);
        _mapperMock.Setup(m => m.Map<RoomDto>(room)).Returns(expectedDto);

        // Act
        var result = await _roomService.Create(creationDto);
    ...
    public async Task Create_ShouldReturnNull_WhenHotelDoesNotExist()
    {
        ...
        _hotelRepositoryMock.Setup(r => r.GetById(-1)).ReturnsAsync((Hotel?)null);
    ...
    public async Task Create_ShouldReturnNull_WhenRepositoryReturnsNull()
    {
        ...
        _hotelRepositoryMock.Setup(r => r.GetById(1)).ReturnsAsync(hotel);
        _mapperMock.Setup(m => m.Map<Room>(creationDto)).Returns(room);
        _roomRepositoryMock.Setup(r => r.Create(It.IsAny<Room>())).ReturnsAsync((Room?)null);
    ...
    public async Task GetById_ShouldReturnDto_WhenRoomExists(int roomId)
    {
        ...
        _roomRepositoryMock.Setup(r => r.GetById(roomId)).ReturnsAsync(room);
    ...
    {
        //Arrange
        _roomRepositoryMock.Setup(r => r.GetById(roomId)).ReturnsAsync((Room?)null);
    ...
    public async Task Delete_ShouldReturnDto_WhenDeletionSucceeds()
    {
        ...
        _roomRepositoryMock.Setup(r => r.Delete(roomId)).ReturnsAsync(room);
        _mapperMock.Setup(m => m.Map<RoomDto>(room)).Returns(new RoomDto()
        {
            Id = roomId,
        });
    ...
    public async Task Delete_ShouldReturnNull_WhenDeletionFails()
    {
        //Arrange
        const int roomId = -1;
        _roomRepositoryMock.Setup(r => r.Delete(roomId)).ReturnsAsync((Room?)null);
