...
public class ReservationServiceTests
{
    ...
    public async Task GetAll_ShouldReturnMappedReservationsWithPagination()
    {
        ...
        _reservationRepositoryMock.Setup(r => r.GetAll(queryParameters))
        ...
        // Act
        var (result, pagination) = await _reservationService.GetAll(queryParameters);
    ...
    public async Task Create_ShouldReturnReservationDto_WhenDataIsValid()
    {
        ...
        _userRepositoryMock.Setup(r => r.GetById(userId)).ReturnsAsync(new User { Id = userId });
        _roomRepositoryMock.Setup(r => r.GetById(roomId)).ReturnsAsync(new Room { Id = roomId });
        _mapperMock.Setup(m => m.Map<Reservation>(creationDto)).Returns(reservationEntity);
        _reservationRepositoryMock.Setup(r => r.Create(reservationEntity)).ReturnsAsync(reservationEntity);
        _mapperMock.Setup(m => m.Map<ReservationDto>(reservationEntity)).Returns(reservationDto);

        // Act
        var result = await _reservationService.Create(creationDto);
    ...
    public async Task Create_ShouldReturnNull_WhenUserIsNotFound()
    {
        ...
        _userRepositoryMock.Setup(r => r.GetById(creationDto.UserId)).ReturnsAsync((User?)null);
    ...
    public async Task Create_ShouldReturnNull_WhenRoomIsNotFound()
    {
        ...
        _userRepositoryMock.Setup(r => r.GetById(creationDto.UserId)).ReturnsAsync(new User { Id = creationDto.UserId });
        _roomRepositoryMock.Setup(r => r.GetById(creationDto.RoomId)).ReturnsAsync((Room?)null);
    ...
    public async Task GetUserIdAndRoomId_ShouldReturnDto_WhenRoomExists(int userId, int roomId)
    {
        ...
        _reservationRepositoryMock.Setup(r => r.GetByUserAndRoomId(userId, roomId)).ReturnsAsync(reservation);
        ...
        //Act
        var result = await _reservationService.GetByUserAndRoomId(userId, roomId);
    ...
    {
        //Arrange
        _reservationRepositoryMock.Setup(r => r.GetByUserAndRoomId(userId, roomId)).ReturnsAsync((Reservation?)null);
    ...
    public async Task Delete_ShouldReturnDto_WhenDeletionSucceeds()
    {
        ...
        _reservationRepositoryMock.Setup(r => r.DeleteByUserAndRoomId(userId, roomId)).ReturnsAsync(reservation);
        _mapperMock.Setup(m => m.Map<ReservationDto>(reservation)).Returns(new ReservationDto
        {
            UserId = userId,
            RoomId = roomId
        ...
        // Act
        var result = await _reservationService.DeleteByUserAndRoomId(userId, roomId);
    ...
    {
        // Arrange
        _reservationRepositoryMock.Setup(r => r.DeleteByUserAndRoomId(userId, roomId)).ReturnsAsync((Reservation?)null);
